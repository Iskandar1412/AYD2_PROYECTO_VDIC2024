...
(
    ...
    puntuacion    int                            null,
    cui           bigint                         not null,
    constraint comentario_cliente_fk
...
(
    cuenta_dolar_id bigint auto_increment
        primary key,
    fecha_apertura  datetime not null,
    cuenta_id       bigint   not null,
    constraint cuenta_dolar__idx
        unique (cuenta_id),
    constraint cuenta_dolar_cuenta_fk
...
(
    ...
    link_papeleria varchar(200)                                                            null,
...
(
    despido_id int auto_increment
        primary key,
    link_doc   varchar(200)                    not null,
    estado     enum ('pendiente', 'realizado') not null,
    cui        bigint                          not null,
    fecha      datetime,
    constraint despido__idx
        unique (cui),
    constraint despido_empleado_fk
...
(
    ...
    cui           bigint       not null,
    constraint supervisor_empleado_fk
        foreign key (cui) references empleado (cui)
);

create table tarjeta
(
    tarjeta_id      bigint auto_increment
        primary key,
    ...
    cuenta_id       bigint                                      not null,
    constraint tarjeta_cuenta_fk
        foreign key (cuenta_id) references cuenta (cuenta_id)
);

create table cancelacion
(
    cancelacion_id  int auto_increment
        primary key,
    ...
    constraint cancelacion__idx
        unique (cuenta_id),
    constraint cancelacion__idxv1
        unique (tarjeta_id),
    constraint cancelacion_cuenta_fk
        foreign key (cuenta_id) references cuenta (cuenta_id),
    constraint cancelacion_tarjeta_fk
        foreign key (tarjeta_id) references tarjeta (tarjeta_id),
    constraint arc_2
        check (((`cuenta_id` is not null) and (`tarjeta_id` is null)) or
               ((`tarjeta_id` is not null) and (`cuenta_id` is null)))
);

create table tipo_prestamo
(
    tipo_prestamo_id int auto_increment
        primary key,
    tipo             varchar(50) not null
);
...
(
    prestamo_id      bigint auto_increment
        primary key,
    monto            decimal(15, 2)                              not null,
    saldo            decimal(15, 2)                              not null,
    ...
    cuenta_id        bigint                                      not null,
    constraint prestamo_cuenta_fk
        foreign key (cuenta_id) references cuenta (cuenta_id),
    constraint prestamo_tipo_prestamo_fk
...
(
    ...
    constraint documento_prestamo_fk
...
(
    ...
    estado         enum ('pendiente', 'aceptado', 'rechazado') not null,
    constraint solicitud__idx
        unique (prestamo_id),
    constraint solicitud__idxv1
        unique (tarjeta_id),
    constraint solicitud__idxv2
        unique (cancelacion_id),
    constraint solicitud_cancelacion_fk
        foreign key (cancelacion_id) references cancelacion (cancelacion_id),
    constraint solicitud_prestamo_fk
        foreign key (prestamo_id) references prestamo (prestamo_id),
    constraint solicitud_tarjeta_fk
        foreign key (tarjeta_id) references tarjeta (tarjeta_id),
    constraint arc_4
        check (((`prestamo_id` is not null) and (`tarjeta_id` is null) and (`cancelacion_id` is null)) or
               ((`tarjeta_id` is not null) and (`prestamo_id` is null) and (`cancelacion_id` is null)) or
               ((`cancelacion_id` is not null) and (`prestamo_id` is null) and (`tarjeta_id` is null)))
);
...
(
    ...
    constraint transaccion_empleado_fk
        foreign key (empleado_cui) references empleado (cui),
    constraint transaccion_moneda_fk
        foreign key (moneda_id) references moneda (moneda_id),
    constraint transaccion_prestamo_fk
        foreign key (prestamo_id) references prestamo (prestamo_id),
    constraint transaccion_retiro_fk
        foreign key (retiro_id) references retiro (retiro_id),
    constraint transaccion_servicio_fk
        foreign key (servicio_id) references servicio (servicio_id),
    constraint transaccion_tarjeta_fk
        foreign key (tarjeta_id) references tarjeta (tarjeta_id),
    constraint arc_3
        check (((`servicio_id` is not null) and (`retiro_id` is null) and (`deposito_id` is null) and
                (`prestamo_id` is null) and (`tarjeta_id` is null)) or
               ...
               ((`prestamo_id` is not null) and (`servicio_id` is null) and (`retiro_id` is null) and
                (`deposito_id` is null) and (`tarjeta_id` is null)) or
               ((`tarjeta_id` is not null) and (`servicio_id` is null) and (`retiro_id` is null) and
                (`deposito_id` is null) and (`prestamo_id` is null)))
);
...
create
    definer = root@`%` procedure DepositarDinero(IN p_monto decimal(15, 2), IN p_fecha datetime, IN p_cuenta_id bigint,
...
create
    definer = root@`%` procedure ExtraerBackups()
BEGIN
    DECLARE v_error_message VARCHAR(255);
...
create
    definer = root@`%` procedure ExtraerSolicitudEmpleados()
BEGIN
    DECLARE v_error_message VARCHAR(255);
...
create
    definer = root@`%` procedure ExtraerTodosLosEmpleados()
BEGIN
    DECLARE v_error_message VARCHAR(255);
...
create
    definer = root@`%` procedure GestionInventario()
BEGIN
    DECLARE v_error_message VARCHAR(255);
...
create
    definer = root@`%` procedure InsertarBackup(IN p_nombre_copia varchar(100), IN p_link_backup varchar(250))
BEGIN
    DECLARE v_error_message VARCHAR(255);
...
create
    definer = root@`%` procedure LoginAdm2fa(IN p_id_admin int, IN p_2fa varchar(200), IN p_cui bigint)
BEGIN
...
create
    definer = root@`%` procedure RegistrarEmpleado(IN p_cui bigint, IN p_nombres varchar(50),
                                                   ...
                                                   IN p_link_foto varchar(200), IN p_link_firma varchar(200),
                                                   IN p_genero enum ('masculino', 'femenino'),
                                                   IN p_estado_civil enum ('soltero', 'casado'),
                                                   IN p_link_papeleria varchar(200))
BEGIN
    DECLARE v_error_message VARCHAR(255);
