<script>
    ...
    let firmaPreview = $state(null);
    let fotoBase64 = $state(null);
    let firmaBase64 = $state(null);

    let papeleria = $state(null);
    let papeleriaNombre = $state(null);

    function handleSendEmail(pass) {
        const mailOptions = {
            from: 'no-reply@proyect1activate.com',
            to: correo,
            subject: 'Creación de cuenta normal',
    ...
    function crearUsuario() {
		...
		.then(async (data) => {
            console.log(data)
			if(data.status === 'success') {
                // await volverAdmin(usuarioTemp.cui);
                alert('Usuario creado exitosamente')
                handleSendEmail(usuarioTemp.password)
                window.location.reload
            }
...
<div class="flex justify-between items-center mb-8">
    ...
    <button
        ...
        disabled={
            !nombres || !apellidos || !dpiFull || !correo || !genero || !estadoCivil ||
            (!edad || edad <= 0) || 
            !papeleria || 
            !fotoBase64 || 
            !firmaBase64
...
<div class="w-3/4 m-auto grid gap-8">
    ...
    <div class="grid grid-cols-3 gap-6">
        ...
        <!-- Firma -->
        <div class="flex flex-col text-center items-center justify-center">
            <label for="firmadigital" class="block text-lg font-medium mb-2 text-gray-700">Subir Firma</label>
            <div id="firmadigital" class="w-32 h-32 border-2 border-dashed border-gray-300 rounded flex items-center justify-center mb-4">
                {#if firmaPreview}
                    <img src={firmaPreview} alt="Firma" class="w-full h-full object-cover rounded" />
                {:else}
                    <span class="text-gray-400 text-sm">Vista previa</span>
                {/if}
            </div>
            ...
            >
                ...
                <input
                    ...
                    accept="image/*"
                    class="hidden"
                    onchange={(e) => handleImageUpload(e, 'firma')}
                />
            </label>
        </div>
